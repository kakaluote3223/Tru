...
import com.computer.hdu.truckrental.tools.Check;
import com.computer.hdu.truckrental.tools.Tool;
...
/*        getSupportActionBar().setHomeButtonEnabled(false);//决定左上角的图标是否可以点击
        ...
        //选择出发地按钮监听事件
        mOrderSelectDeparture.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(OrderCreateActivity.this, LocationSelectActivity.class);
                //TODO:判断现在的出发地 目的地   再传入
                startActivityForResult(intent, 1);
            }
        });

        //选择目的地按钮监听事件
        mOrderSelectDestination.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(OrderCreateActivity.this, LocationSelectActivity.class);
                //TODO:判断现在的出发地 目的地   再传入
        ...
        mOrderCreateBtn.setOnClickListener(new View.OnClickListener() {
            //获取当前日期
            String startDate = Tool.getCurrentTime();
            @Override
    ...
    private Integer OrderCreate(String startDate){
        ...
        String dateTime = Tool.getCurrentTime();//需要计算 订单生成时间
        //用户id
        Integer fkUserId = 0;//上一个活动传到这个活动中,这里最好设置为私有成员
        order.setFk_user_id(fkUserId);
        //订单号
        order.setOrder_number(Tool.getOrderNumber(startDate, fkUserId.toString()));
        //出发地
        ...
        switch (requestCode){
            case 1:
                if (resultCode == RESULT_OK){
                    //TODO:检测两个按钮是否有地址 有 计算距离 没有不做任何操作
                    String departure = data.getStringExtra("departure");
                    String destination = data.getStringExtra("destination");
                    mOrderSelectDeparture.setText(departure);
                    mOrderSelectDestination.setText(destination);
                    orderDistance = data.getFloatExtra("distance", 0);
                }
                break;
            case 2:
                if (resultCode == RESULT_OK){
